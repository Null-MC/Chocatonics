...
vec4 BilateralUpscale(sampler2D tex, vec2 coord, float frDepth) {
    vec4 vl = vec4(0.0);
    float sum = 0.0;
    mat3x3 weights;
...
//    float dsample = texelFetch(depth, tcDepth, 0).r;
//    dsample = ld_reverse(dsample, zMults.y, zMults.z);
    float dsample = sampleDepth(tcDepth);
...
//        float frDepth = z > 0.0 ? near / z : farPlane;
    ...
    vec4 vl = BilateralUpscale(colortex0, gl_FragCoord.xy, frDepth);
    ...
    bool iswater = trpData.a > 0.99;
        ...
        vec3 viewPos = toScreenSpace_Lod(vec3(texcoord, z));
